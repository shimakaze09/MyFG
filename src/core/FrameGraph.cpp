...
size_t FrameGraph::RegisterPassNode(std::string name,
                                    std::vector<size_t> inputs,
                                    std::vector<size_t> outputs) {
  return RegisterPassNode(
      PassNode{std::move(name), std::move(inputs), std::move(outputs)});
}
...
bool FrameGraph::IsMovedOut(size_t src) const {
  return srcRsrcNodeIdx2moveNodeIdx.find(src) !=
         srcRsrcNodeIdx2moveNodeIdx.end();
}

bool FrameGraph::IsMovedIn(size_t dst) const {
  return dstRsrcNodeIdx2moveNodeIdx.find(dst) !=
         dstRsrcNodeIdx2moveNodeIdx.end();
}
...
MyGraphviz::Graph FrameGraph::ToGraphvizGraph() const {
  MyGraphviz::Graph graph(name, true);
  ...
  subgraph_rsrc.RegisterGraphNodeAttr("color", "#F79646")
      .RegisterGraphNodeAttr("shape", "box");

  subgraph_pass.RegisterGraphNodeAttr("shape", "ellipse")
  ...
  for (const auto& passNode : passNodes) {
    ...
    for (size_t rsrcNodeIndex : passNode.Inputs()) {
      ...
      auto edgeIndex =
          registry.RegisterEdge(registry.GetNodeIndex(rsrcNodeName), passIndex);
    ...
    for (size_t rsrcNodeIndex : passNode.Outputs()) {
      const auto& rsrcNodeName = resourceNodes[rsrcNodeIndex].Name();
      auto edgeIndex =
          registry.RegisterEdge(passIndex, registry.GetNodeIndex(rsrcNodeName));
      subgraph_write.AddEdge(edgeIndex);
    }
  }

  for (const auto& moveNode : moveNodes) {
    auto srcNodeIdx = registry.GetNodeIndex(
        resourceNodes[moveNode.GetSourceNodeIndex()].Name());
    auto dstNodeIdx = registry.GetNodeIndex(
        resourceNodes[moveNode.GetDestinationNodeIndex()].Name());
